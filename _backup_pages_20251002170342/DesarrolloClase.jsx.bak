...
import { getDoc, doc, collection, getDocs, query, where } from "firebase/firestore"; // â¬…ï¸ +collection/getDocs/query/where
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ PDF (react-pdf) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
import { Document, Page, pdfjs } from "react-pdf";
// Usar rutas sin `/esm/` para tu versiÃ³n:
import "react-pdf/dist/Page/AnnotationLayer.css";
import "react-pdf/dist/Page/TextLayer.css";

/* ðŸ‘‡ CAMBIO: importar el worker ESM (.mjs) y asignarlo */
import workerSrc from "pdfjs-dist/build/pdf.worker.min.mjs?url";
pdfjs.GlobalWorkerOptions.workerSrc = workerSrc;

/* ===== Copias originales (NO eliminadas) â€” comentadas para evitar colisiÃ³n ===== */
// import "react-pdf/dist/Page/AnnotationLayer.css";
// import "react-pdf/dist/Page/TextLayer.css";
// import pdfWorker from "pdfjs-dist/build/pdf.worker.min.js?url";
// pdfjs.GlobalWorkerOptions.workerSrc = pdfWorker;
//
// /* â¬…ï¸ FIX: usa las rutas sin `esm` (tu paquete no trae esa carpeta) */
// import "react-pdf/dist/Page/AnnotationLayer.css";
// import "react-pdf/dist/Page/TextLayer.css";
// import pdfWorker from "pdfjs-dist/build/pdf.worker.min.js?url";
// pdfjs.GlobalWorkerOptions.workerSrc = pdfWorker;
...
// ðŸ”¹ auth para garantizar sesiÃ³n antes de tocar Firestore
...
/* â¬‡ï¸ NUEVO: base del proxy (mÃ¡s robusto) */
const PROXY_BASE =
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ 
   ðŸ”¸ helpers de tiempo/slot para demo (?at=HH:MM&dow=1..5)
   â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
/* ðŸ”§ Acepta varios formatos en horarioConfig */
...
/* CRONÃ“METRO COMPARTIDO (clave por slot) */
...
/* ðŸ”¤ helpers de normalizaciÃ³n y mapeo a slugs API MINEDUC */
const norm = (s = "") =>
...
function nivelToApi(cursoStr = "", prefer = "") {
  // si dice "bÃ¡sico" -> basica, si no -> media
...
function cursoFromNivelSeccion(nivel = "", seccion = "") {
  const n = (nivel || "").replace(/Âº/g, "Â°");
  return [n, seccion].filter(Boolean).join(" ").trim();
}

/* â¬‡ï¸ Permite forzar slot vÃ­a ?slot=fila-col */
...
/* ===============================
   ðŸ†• CONFIG: catÃ¡logos de Apps
   =============================== */
// (Se conserva, no se elimina)
const APP_GROUPS = {
  Generales: [
    { id:"youtube",  name:"YouTube",               url:"https://youtube.com",              icon:"â–¶", embed:true },
    ...
    { id:"drive",    name:"Drive",                 url:"https://drive.google.com",         icon:"ðŸ“", embed:false },
    { id:"flipbook", name:"Flipbook",              url:"https://heyzine.com",              icon:"ðŸ“–", embed:true },
    { id:"timer",    name:"Timer",                 url:"https://timer.onlineclock.net",    icon:"â±ï¸", embed:true },
    { id:"qr",       name:"QR Generator",          url:"https://qrcode-monkey.com",        icon:"ðŸ”³", embed:true },
  ],
  "MatemÃ¡tica": [
    { id:"geogebra-graph",  name:"GeoGebra Â· Graficadora", url:"https://www.geogebra.org/graphing", icon:"ðŸ“ˆ", embed:true },
    { id:"geogebra-geom",   name:"GeoGebra Â· GeometrÃ­a",   url:"https://www.geogebra.org/geometry", icon:"ðŸ“", embed:true },
    { id:"desmos",          name:"Desmos",                 url:"https://www.desmos.com/calculator", icon:"ðŸ§®", embed:true },
    { id:"phet-math",       name:"PhET MatemÃ¡ticas",       url:"https://phet.colorado.edu/es/simulations/filter?subjects=math", icon:"ðŸ§ª", embed:false },
  ],
  Lenguaje: [
    { id:"wordreference", name:"WordReference",   url:"https://www.wordreference.com", icon:"ðŸ“š", embed:true },
    { id:"linguee",       name:"Linguee",        url:"https://www.linguee.es",        icon:"ðŸ…»", embed:true },
    { id:"docs",          name:"Google Docs",    url:"https://docs.google.com",       icon:"ðŸ“„", embed:false },
    { id:"padlet",        name:"Padlet",         url:"https://padlet.com",            icon:"ðŸ§±", embed:false },
  ],
  Ciencias: [
    { id:"phet-physics",  name:"PhET FÃ­sica",    url:"https://phet.colorado.edu/es/simulations/filter?subjects=physics", icon:"âš›ï¸", embed:false },
    { id:"phet-chem",     name:"PhET QuÃ­mica",   url:"https://phet.colorado.edu/es/simulations/filter?subjects=chemistry", icon:"ðŸ§«", embed:false },
    { id:"phet-bio",      name:"PhET BiologÃ­a",  url:"https://phet.colorado.edu/es/simulations/filter?subjects=biology", icon:"ðŸ§¬", embed:false },
  ],
  Historia: [
    { id:"google-arts",   name:"Google Arts & Culture", url:"https://artsandculture.google.com", icon:"ðŸ›ï¸", embed:true },
    { id:"timeline",      name:"KnightLab Timeline",    url:"https://timeline.knightlab.com",    icon:"ðŸ—“ï¸", embed:false },
  ],
  InglÃs: [
    { id:"cambridge", name:"Cambridge Dictionary", url:"https://dictionary.cambridge.org", icon:"ðŸ‡¬ðŸ‡§", embed:true },
    { id:"reverso",   name:"Reverso Context",      url:"https://context.reverso.net",     icon:"ðŸ”„", embed:true },
  ],
  Artes: [
    { id:"sketchpad", name:"Sketchpad",     url:"https://sketch.io/sketchpad", icon:"ðŸŽ¨", embed:true },
    { id:"autodraw",  name:"AutoDraw",      url:"https://www.autodraw.com",    icon:"âœï¸", embed:true },
  ],
  Tecnologia: [
    { id:"scratch", name:"Scratch",         url:"https://scratch.mit.edu",           icon:"ðŸ±", embed:false },
    { id:"w3schools", name:"W3Schools",     url:"https://www.w3schools.com/tryit",   icon:"ðŸ’»", embed:true },
  ],
};

/* ðŸŽ¨ estilos grid Apps */
const appsWrap = {
  marginTop: 10,
  paddingTop: 10,
  borderTop: `1px dashed ${COLORS.border}`,
...
export default function DesarrolloClase({ duracion = 30, onIrACierre }) {
  ...
  // âœ… useContext dentro del componente
  ...
  // presentaciÃ³n (archivo o pantalla)
  ...
  const ytEmbedFromQuery = (q) => {
    ...
    return `https://www.youtube-nocookie.com/embed?listType=search&list=${encodeURIComponent(q || "matemÃ¡tica 1 medio")}`;
  };

  const toEmbedURL = (urlOrQuery, kind) => {
    if (kind === "youtube") {
      const q = (urlOrQuery || "").trim() || (unidad ? `matemÃ¡tica ${unidad}` : "matemÃ¡tica 1 medio");
    ...
    try {
      ...
      // â¬‡ï¸ CAMBIO: Wikipedia ya NO usa readerWrap (para que se vea como Wikipedia)
  ...
  const trySetMain = (embedUrl) => {
    ...
    if (
      ...
      setPanelMsg(
        "Este sitio puede requerir inicio de sesiÃ³n o bloquear funciones dentro del panel. Si no ves contenido, pega un enlace pÃºblico/â€œembedâ€."
      );
    }
  };

  // PresentaciÃ³n de archivo o pantalla
  ...
  const handleFileChosen = (e) => {
    ...
    if (file.type === "application/pdf") {
      ...
      alert("Para Word/Excel/PowerPoint, usa Â«Compartir pantallaÂ». (El panel renderiza nativamente PDF e imÃ¡genes).");
    }
  ...
  const handleOpenPhET = () => {
    ...
    setPanelMsg(
      "Para ver una simulaciÃ³n dentro del panel, entra a la simulaciÃ³n â†’ 'Compartir/Insertar' y pega ese enlace en 'Abrir en panel'."
    );
  };
  ...
  // â¬‡ï¸ CAMBIO: abrir Wikipedia DIRECTA (sin readerWrap)
  ...
  // ðŸ” asegura sesiÃ³n anÃ³nima primero
  useEffect(() => {
  ...
  // ðŸ”„ AUTOâ€“INICIO cronÃ³metro si no hay endMs vÃ¡lido
  useLayoutEffect(() => {
    try {
  ...
  useEffect(() => {
    ...
    const obtenerDatos = async () => {
      try {
        ...
        // 3) fallback clases_detalle/{uid}/slots/Ãºltimo o por ?slot=
        try {
          const slotPref =
            location?.state?.slotId || slotFromQuery() || localStorage.getItem("__lastSlotId") || "0-0";
        ...
        const asigForOA =
          location?.state?.clase?.asignatura || asignatura || "MatemÃ¡tica";
        const cursoForOA =
        ...
        if (unidadParaOA) {
          ...
          try {
            // 1) buscamos tÃ­tulo por unidad (fallback objetivo)
    ...
    obtenerDatos();
  }, [authed]); // solo cuando ya hay sesiÃ³n

  /* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ 
     ðŸ”¸ marcas â†’ clases_detalle
     â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  useEffect(() => {
    if (!authed) return;
    (async () => {
      try {
        ...
        console.warn("[Desarrollo] marcasâ†’clases_detalle:", e?.code || e?.message);
      }
    })();
  }, [authed]); // eslint-disable-line

  // al terminar el cronÃ³metro, ir a cierre
  const handleEnd = () => {
  ...
  const openShapE = () => {
    ...
    setPanelMsg("Usa la Space para generar y descargar el GLB; luego pÃ©galo abajo en â€˜URL GLBâ€™ o sÃºbelo.");
  };
  const openTripoSR = () => {
    trySetMain(TRIPOSR_SPACE_URL);
    setPanelMsg("Primero genera una imagen (p. ej., en Stable Diffusion) y sÃºbala en la Space para obtener el GLB.");
  };
  ...
  // generaciÃ³n vÃ­a servidor local (proxy)
  ...
  const generateLocal3D = async () => {
    ...
    try {
      ...
      console.warn(err);
      setGenMsg("No se pudo generar desde el servidor local. Abre Shap-E (botÃ³n) o verifica tu proxy en " + PROXY_BASE);
      alert("Fallo la generaciÃ³n local. Revisa el proxy o usa el botÃ³n de Shap-E.");
    } finally {
      setGenLoading(false);
  ...
  const openEscapeDemo = () => {
    ...
    setPanelMsg("Demo A-Frame. Para una sala propia, pega la URL de tu mundo (FrameVR/Spatial/etc.) y Ã¡brela.");
  };
  ...
  // â¬‡ï¸ NUEVO: modo (flipbook/scroll) + URL resuelta desde mineduc_pdfs
  const [pdfMode, setPdfMode] = useState("flip"); // 'flip' | 'scroll'
  const [resolvedMineducUrl, setResolvedMineducUrl] = useState("");

  // Nivel heurÃ­stico (desde ficha, claseVigente o curso)
  const nivelPrefer = (() => {
    const fromState = location?.state?.ficha?.nivel || "";
    const fromVig = claseVigente?.nivel || "";
    if (fromState) return fromState;
    if (fromVig) return fromVig;
    const m = String(curso || "").match(/(\d+)\s*[Â°Âº]?\s*(b[aÃ¡]sico|medio)/i);
    return m ? `${m[1]}Â° ${m[2].toLowerCase().replace("basico", "bÃ¡sico")}` : "";
  })();
  ...
  /* ===============================
     ðŸ†• LÃ“GICA Apps Grid (catÃ¡logo)
     =============================== */
  ...
  // â¬‡ï¸ NUEVO: oculta los Ã­conos duplicados (YouTube/GeoGebra/Desmos/PhET/Drive/Wiki)
    ...
    <div style={page}>
        ...
        >
          ...
          {claseVigente.unidad && (
            <div>
              ...
              {claseVigente.evaluacion ? " Â· (EvaluaciÃ³n)" : ""}
            </div>
          )}
        </div>
      )}
      ...
      <div style={{ ...row("1rem"), marginBottom: "1rem" }}>
        ...
        <div style={card}>
          ...
          <div style={{ color: COLORS.textMuted, marginBottom: 8 }}>ðŸ•’ {horaActual}</div>
        ...
        <div style={{ ...card, display: "flex", flexDirection: "column", gap: ".6rem" }}>
          <div style={{ display: "flex", gap: ".5rem" }}>
            <button style={btnWhite}>âœ‹</button>
            <button style={btnWhite}>ðŸ‘</button>
          </div>
      ...
      <div style={{ ...card, textAlign: "center", marginBottom: "1rem" }}>
        ...
        {panelMsg && <div style={{ marginTop: 8, color: "#b45309" }}>âš ï¸ {panelMsg}</div>}

        {/* Herramientas rÃ¡pidas */}
        <div style={toolsWrap}>
          <h4 style={{ margin: "0 0 8px" }}>ðŸ”§ Herramientas rÃ¡pidas</h4>
          ...
          <div style={toolsRow}>
            ...
            <input
              ...
              placeholder="Buscar o pegar enlace de YouTubeâ€¦ (ej: proporcionalidad o https://youtu.be/ID)"
            />
            ...
            >
              <option value="calculator">GeoGebra Â· Graficadora</option>
              <option value="geometry">GeoGebra Â· GeometrÃ­a</option>
              <option value="3d">GeoGebra Â· 3D</option>
            </select>
            ...
            {/* Link personalizado â†’ en panel */}
            <input
              style={{ ...inputSmall, minWidth: 280 }}
              value={customLink}
              onChange={(e) => setCustomLink(e.target.value)}
              placeholder="Pega cualquier enlace (intentaremos mostrarlo aquÃ­)"
            />
          ...
          {/* barra de Ã­conos â€” Duplicados ocultos */}
          <div style={iconsRow}>
            ...
            <button
              ...
              title={sharing ? "Compartiendoâ€¦ (elige otra opciÃ³n)" : "Presentar (pantalla o archivo)"}
            >
              <IconPresent />
            </button>
            ...
            <button style={iconBtn} onClick={openShapE} title="Shap-E (textoâ†’3D)">
              <img src={ico("huggingface.co")} alt="Shap-E" style={iconImg} />
            </button>
            <button style={iconBtn} onClick={openTripoSR} title="TripoSR (imagenâ†’3D)">
            ...
            <div style={{ ...toolsRow, justifyContent: "space-between" }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                ...
                <input
                  ...
                  placeholder="Buscar app por nombre/categorÃ­aâ€¦"
                />
              </div>
            </div>
              ...
              <div style={appsGrid}>
                {appsFiltered.map((app) => {
                  ...
                  return (
                    <div
                      ...
                      style={appTile}
                      onClick={() => openApp(app)}
                      title={app.embed ? "Abrir dentro del panel" : "Abrir en pestaÃ±a nueva"}
                    >
                      <img src={ico(domain)} alt={app.name} style={appIcon} />
                      <div style={{ display: "flex", flexDirection: "column" }}>
                        <span style={appTitle}>
                          {app.name} {app.embed ? "" : "â†—"}
                        </span>
                ...
                {appsFiltered.length === 0 && (
                  <div style={{ gridColumn: "1/-1", color: COLORS.textMuted, fontStyle: "italic", padding: 6 }}>
                    Sin resultados para ese filtro/bÃºsqueda.
                  </div>
                )}
              </div>
            )}
          ...
          {/* input de archivo oculto para PDF/imagen (presentaciÃ³n) */}
          <input
            ref={fileInputRef}
            type="file"
        ...
        {/* FIN herramientas rÃ¡pidas */}
      </div>

      {/* Visor PDF del Programa */}
      <div style={{ ...card, marginBottom: "1rem" }}>
        <h3 style={{ marginTop: 0, textAlign: "center" }}>ðŸ“„ Programa de Estudio (PDF)</h3>
        ...
        <div style={{ ...toolsRow, marginBottom: 8 }}>
          <input
            ...
            placeholder="Pega aquÃ­ la URL del PDF del programa (nivel + asignatura)"
          />
        ...
        {!finalPdfUrl ? (
          <div style={{ color: COLORS.textMuted, textAlign: "center" }}>
            Pega la URL del PDF arriba. Si vienes desde <em>Inicio</em> con ficha, uso automÃ¡ticamente{" "}
            <code>ficha.programaUrl</code>. Si estÃ¡ vacÃ­o, intentamos buscarlo en <code>mineduc_pdfs</code> por tu{" "}
            {nivelPrefer ? `nivel Â«${nivelPrefer}Â»` : "nivel"} y asignatura.
          </div>
        ) : pdfMode === "scroll" ? (
          <div
            ref={pdfWrapRef}
          ...
          >
            ...
            >
              <Document
                ...
                loading={<div style={{ padding: 12 }}>Cargando PDFâ€¦</div>}
              >
      ...
      {/* Laboratorio 3D */}
      <div style={{ ...card, marginBottom: "1rem" }}>
        <h3 style={{ marginTop: 0, textAlign: "center" }}>ðŸ§ª Laboratorio 3D</h3>
        ...
        <div style={{ ...toolsRow, marginBottom: 6 }}>
          ...
          <button style={btnTool} onClick={openShapE}>
            ðŸš€ Abrir Shap-E (textoâ†’3D)
          </button>
          <button style={btnTool} onClick={openTripoSR}>
            ðŸ–¼ï¸ Abrir TripoSR (imagenâ†’3D)
          </button>
        ...
        <div style={{ ...toolsRow, marginBottom: 10 }}>
          <input
            ...
            placeholder="URL .glb/.gltf (pÃ©galo aquÃ­ para previsualizar)"
          />
        ...
        <div ref={viewerRef} style={{ display: "flex", justifyContent: "center" }}>
          {modelSrc ? (
            ...
            <div style={{ color: COLORS.textMuted, fontStyle: "italic" }}>
              Pega una URL .glb/.gltf o sube un GLB para previsualizarlo aquÃ­.
            </div>
          )}
        </div>
      </div>

      {/* ðŸ†• Intro de Wikipedia (3â€“4 lÃ­neas) */}
      {wikipediaUrl && (wikiSummary || wikiTitle) && (
        <div style={{ ...card, marginBottom: "0.75rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 6 }}>
            <h3 style={{ margin: 0 }}>ðŸ“š IntroducciÃ³n (Wikipedia)</h3>
            <div style={subtle}>
              Relacionado con: <strong>{unidad || objetivo || "la temÃ¡tica de la clase"}</strong>
            </div>
          </div>
          ...
          <div style={wikiIntroWrap}>
            ...
            <div>
              ...
              {wikiSummary ? (
                ...
                <div style={{ color: COLORS.textMuted, fontStyle: "italic" }}>
                  No se pudo obtener el resumen. Puedes abrir el artÃ­culo completo abajo.
                </div>
              )}
      ...
      {/* ArtÃ­culo de Wikipedia embebido (bonito, sin readerWrap) */}
      {wikipediaUrl && (
        <div style={{ ...card }}>
          <h3 style={{ marginTop: 0, textAlign: "center" }}>ðŸ“š InformaciÃ³n complementaria</h3>
          <iframe
            src={wikipediaUrl} // â¬…ï¸ directo, sin r.jina.ai
            width="100%"
            height="420"
