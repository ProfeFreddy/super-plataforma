...
export default function CierreClase({
  ...
  const mergeClase = (res) => {
    ...
    setClaseActual((prev) => ({
      ...
      curso:
        res?.curso ??
        prev?.curso ??
        (res?.nivel && res?.seccion ? `${res.nivel}${res.seccion}` : "(sin curso)"),
      profesor: res?.profesor ?? prev?.profesor ?? profesor ?? "Profesor/a",
    }));
  };
  ...
  useEffect(() => {
    ...
    setClaseVigente({
      ...
      habilidades: st?.ficha?.habilidades || st?.clase?.habilidades || null,
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
    ...
    >
      <div style={page}>
        ...
        {claseVigente ? (
          <Card
            style={{
              background: "linear-gradient(180deg, #46b6e6 0%, #2aa5d6 100%)",
              color: "white",
              padding: 18,
            }}
          >
        ...
        >
          ...
          <Card>
            ...
            <div style={{ fontWeight: 800, marginBottom: 8 }}>{unidadShow}</div>
            <div style={{ ...label, marginBottom: 8 }}>
              <strong>Objetivo:</strong> {objetivoShow}
            </div>
            <div style={label}>
              <strong>Curso:</strong> {cursoShow} &nbsp; | &nbsp;
              <strong>Asignatura:</strong> {asignaturaShow}
            </div>
          </Card>
