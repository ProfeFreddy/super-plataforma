...
useEffect(() => {
  (async () => {
    try {
      ...
      if (res) {
        // === FIX SEGURO: normaliza todo en un solo set ===
        setClaseActual((prev) => ({
          ...(prev || {}),
          ...
          // si tienes el helper para armar el curso desde nivel+sección, úsalo:
          curso:
            (typeof cursoFromNivelSeccion === "function"
              ? cursoFromNivelSeccion(res?.nivel, res?.seccion)
              : undefined) ??
            prev?.curso ??
            "(sin curso)",
        }));

        // (Opcional) Si otras partes del componente todavía leen estos
        // estados sueltos (unidad/objetivo/habilidades/asignatura/curso),
        // puedes mantenerlos sincronizados SIN borrar el código antiguo:
        /*
        setUnidad((u) => res?.unidad ?? u ?? "(sin unidad)");
        setObjetivo((o) => res?.objetivo ?? o ?? "(sin objetivo)");
        setHabilidades((h) =>
          Array.isArray(res?.habilidades) ? res.habilidades.join(", ") :
          (res?.habilidades ?? h ?? "(sin habilidades)")
        );
        setAsignatura((a) => res?.asignatura ?? a ?? "(sin asignatura)");
        setCurso((c) =>
          (typeof cursoFromNivelSeccion === "function"
            ? cursoFromNivelSeccion(res?.nivel, res?.seccion)
            : undefined) ?? c ?? "(sin curso)"
        );
        */
      }
    } catch (e) {
